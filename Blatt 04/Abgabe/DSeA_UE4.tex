...
Bew: \\
Sei die Menge aller $x_i$ sortiert, sonst führe eine Permutation aus, so das die Menge aller $x_i$ sortiert ist. \\
%Teile die Summe in zwei gleich lange Teilsummen, wobei Summe 1 alle $x_i$ kleiner als $x_{k+1}$ und Summe 2 alle $x_i$ größer als $x_{k+1} $ abdeckt:\\
\underline{1. Fall $x \neq x_i~\forall~ i\in[1,n]:$}
\[\sum_{i=1}^{2k+1}|x_i-x|= \sum_{i=1}^{j-1}|x_i-x| + \sum_{i=j+1}^{2k+1}|x_i-x| + |x_{j}-x|,~j\in[1,n]\]
\underline{2. Fall $\exists~ j\in[1,n]\setminus\{k\}:~x = x_j:$}\\
\[\sum_{i=1}^{2k+1}|x_i-x|= \sum_{i=1}^{j-1}|x_i-x| + \sum_{i=j+1}^{2k+1}|x_i-x| + |x_{j}-x|= \sum_{i=1}^{j-1}|x_i-x| + \sum_{i=j+1}^{2k+1}|x_i-x|\]
\[=(j-1)x-\sum_{i=1}^{j-1}x_i-(2k+2-j-1)x+\sum_{i=j+1}^{2k+1}x_i=(j-1)x-(2k-j+1)x+\sum_{i=j+1}^{2k+1}x_i -\sum_{i=1}^{j-1}x_i\]
\[=(2j-2k-2)x+\sum_{i=j+1}^{2k+1}x_i -\sum_{i=1}^{j-1}x_i=2(j-1-k)x+\sum_{i=j+1}^{2k+1}x_i -\sum_{i=1}^{j-1}x_i\leq \text{1. Fall} \]
\underline{2. Fall ist minimal, wenn $(j-1-k)=0 \Leftrightarrow j = k+1$}
